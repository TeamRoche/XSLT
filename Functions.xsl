...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:isc="http://extension-functions.intersystems.com" xmlns:hl7="urn:hl7-org:v3" xmlns:sdtc="urn:hl7-org:sdtc" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:exsl="http://exslt.org/common" exclude-result-prefixes="isc hl7 sdtc xsi exsl">
	...
	<xsl:template match="*" mode="CodeTable">
		...
		-->
		<xsl:variable name="firstTranslationCodeSystem">
	...
	<xsl:template match="*" mode="CodeTableDetail">
		...
		<xsl:variable name="originalText">
			<xsl:choose>
				<xsl:when test="string-length(normalize-space($referenceValue))">
			...
			<xsl:choose>
				<xsl:when test="not($useFirstTranslation='1')">
					<xsl:choose>
						...
						<xsl:when test="string-length(normalize-space($referenceValue))">
							<xsl:value-of select="normalize-space($referenceValue)"/>
						</xsl:when>
						<xsl:when test="string-length(normalize-space(hl7:originalText/text()))">
							<xsl:value-of select="normalize-space(hl7:originalText/text())"/>
						</xsl:when>
						<xsl:when test="($hsElementName='OrderItem') and string-length($orderItemDefaultCode)">
							<xsl:value-of select="$orderItemDefaultCode"/>
						</xsl:when>
					</xsl:choose>
				</xsl:when>
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="hl7:translation[1]/@xsi:type = 'ST'">
							<xsl:value-of select="normalize-space(hl7:translation[1]/text())"/>
						</xsl:when>
						<xsl:when test="hl7:translation[1]/@code">
							<xsl:value-of select="translate(hl7:translation[1]/@code, '_', ' ')"/>
						</xsl:when>
						<xsl:when test="string-length(normalize-space($referenceValue))">
							<xsl:value-of select="normalize-space($referenceValue)"/>
						</xsl:when>
		...
		<xsl:variable name="description">
			<xsl:choose>
				<xsl:when test="not($useFirstTranslation='1')">
					<xsl:choose>
						<xsl:when test="string-length(normalize-space(@displayName))">
						...
						<xsl:when test="normalize-space(hl7:originalText/text())">
							<xsl:value-of select="normalize-space(hl7:originalText/text())"/>
						</xsl:when>
						<xsl:when test="($hsElementName='OrderItem') and string-length($orderItemDefaultDescription)">
							<xsl:value-of select="$orderItemDefaultDescription"/>
						</xsl:when>
					</xsl:choose>
				</xsl:when>
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="string-length(normalize-space(hl7:translation[1]/@displayName))">
							<xsl:value-of select="normalize-space(hl7:translation[1]/@displayName)"/>
						</xsl:when>
						<xsl:when test="hl7:translation[1]/@xsi:type = 'ST'">
							<xsl:value-of select="normalize-space(hl7:translation[1]/text())"/>
						</xsl:when>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:if test="string-length($codingStandard)">
					<NVPair>
						<Name>
							<xsl:value-of select="concat($hsElementName,'CodingStandard')"/>
						</Name>
						<Value>
							<xsl:value-of select="$codingStandard"/>
						</Value>
				...
				<NVPair>
					<Name>
						<xsl:value-of select="concat($hsElementName,'Code')"/>
					</Name>
					<Value>
						<xsl:value-of select="$code"/>
					</Value>
				...
				<NVPair>
					<Name>
						<xsl:value-of select="concat($hsElementName,'Description')"/>
					</Name>
					<Value>
						<xsl:value-of select="$description"/>
					</Value>
	...
	<xsl:template match="*" mode="SendingFacility">
		<xsl:variable name="organizationInformation">
			<xsl:apply-templates select="hl7:assignedAuthor/hl7:representedOrganization" mode="OrganizationInformation"/>
		</xsl:variable>
		...
		<xsl:if test="string-length($organizationInformation)">
			...
			<SendingFacility>
				<xsl:choose>
					<xsl:when test="string-length($facilityCode)">
						<xsl:value-of select="$facilityCode"/>
					</xsl:when>
					<xsl:when test="string-length($facilityDescription)">
						<xsl:value-of select="$facilityDescription"/>
					</xsl:when>
	...
	<xsl:template match="*" mode="SendingFacilityValue">
		...
		-->
		<xsl:choose>
			<xsl:when test="$defaultInformantRootPath/hl7:assignedEntity/hl7:representedOrganization">
				<xsl:apply-templates select="$defaultInformantRootPath" mode="SendingFacilityFromOrgInfo"/>
			</xsl:when>
			<xsl:when test="$defaultAuthorRootPath/hl7:assignedAuthor/hl7:representedOrganization">
				<xsl:apply-templates select="$defaultAuthorRootPath" mode="SendingFacilityFromOrgInfo"/>
			</xsl:when>
			<xsl:when test="$defaultAuthoringDeviceRootPath/hl7:assignedAuthor/hl7:representedOrganization">
				<xsl:apply-templates select="$defaultAuthoringDeviceRootPath" mode="SendingFacilityFromOrgInfo"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates select="." mode="code-for-oid">
					<xsl:with-param name="OID" select="/hl7:ClinicalDocument/hl7:recordTarget/hl7:patientRole/hl7:id/@root"/>
				</xsl:apply-templates>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template match="*" mode="SendingFacilityFromOrgInfo">
		<xsl:variable name="organizationInformation">
			<xsl:apply-templates select=".//hl7:representedOrganization" mode="OrganizationInformation"/>
		</xsl:variable>
	...
	<xsl:template match="*" mode="PerformedAt">
		...
		-->
		<xsl:apply-templates select="hl7:assignedEntity" mode="OrganizationDetail">
			<xsl:with-param name="elementName" select="'PerformedAt'"/>
		</xsl:apply-templates>
	</xsl:template>
	...
	<xsl:template match="*" mode="EnteredAt">
		...
		<xsl:choose>
			<xsl:when test="hl7:informant">
				<xsl:apply-templates select="hl7:informant" mode="OrganizationDetail"/>
			</xsl:when>
			<xsl:when test="hl7:author">
				<xsl:apply-templates select="hl7:author" mode="OrganizationDetail"/>
			</xsl:when>
			<xsl:when test="local-name()='informant'">
				<xsl:apply-templates select="." mode="OrganizationDetail"/>
			</xsl:when>
			<xsl:when test="local-name()='author' and not(hl7:assignedAuthor/hl7:assignedAuthoringDevice)">
				<xsl:apply-templates select="." mode="OrganizationDetail"/>
			</xsl:when>
			<xsl:when test="$defaultInformantRootPath">
				<xsl:apply-templates select="$defaultInformantRootPath" mode="OrganizationDetail"/>
			</xsl:when>
			<xsl:when test="$defaultAuthorRootPath">
				<xsl:apply-templates select="$defaultAuthorRootPath" mode="OrganizationDetail"/>
			</xsl:when>
			<xsl:when test="$defaultAuthoringDeviceRootPath">
				<xsl:apply-templates select="$defaultAuthoringDeviceRootPath" mode="OrganizationDetail"/>
			</xsl:when>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template match="*" mode="EnteringOrganization">
		...
		<xsl:choose>
			<xsl:when test="hl7:informant">
				<xsl:apply-templates select="hl7:informant" mode="HealthCareFacilityDetail">
					<xsl:with-param name="elementName" select="'EnteringOrganization'"/>
				</xsl:apply-templates>
			</xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates select="$defaultInformantRootPath" mode="HealthCareFacilityDetail">
					<xsl:with-param name="elementName" select="'EnteringOrganization'"/>
				</xsl:apply-templates>
			</xsl:otherwise>
		</xsl:choose>
	...
	<xsl:template match="*" mode="HealthCareFacility">
		<xsl:apply-templates select="." mode="HealthCareFacilityDetail">
	...
	<xsl:template match="*" mode="HealthCareFacilityDetail">
		...
		<xsl:variable name="organizationInformation">
			<xsl:apply-templates select=".//hl7:representedOrganization | .//hl7:serviceProviderOrganization" mode="OrganizationInformation"/>
		</xsl:variable>

		<xsl:variable name="locationInformation">
			<xsl:apply-templates select="hl7:healthCareFacility" mode="LocationInformation"/>
		</xsl:variable>

		<xsl:variable name="sendingFacilityCode">
			<xsl:apply-templates select="/hl7:ClinicalDocument" mode="SendingFacilityValue"/>
		</xsl:variable>
		...
		<xsl:element name="{$elementName}">
			<xsl:choose>
				<xsl:when test="$locationCode">
					...
					<xsl:if test="string-length($locationDescription)">
						<Description>
				...
				<xsl:when test="$facilityCode">
					...
					<xsl:if test="string-length($facilityDescription)">
						<Description>
							<xsl:value-of select="$facilityDescription"/>
						</Description>
					</xsl:if>
				...
				<xsl:when test="$sendingFacilityCode">
					<Code>
						<xsl:value-of select="$sendingFacilityCode"/>
					</Code>
					<Description>
						<xsl:value-of select="$sendingFacilityCode"/>
					</Description>
	...
	<xsl:template match="*" mode="EnteredBy">
		<xsl:choose>
			<xsl:when test="hl7:author">
				<xsl:apply-templates select="hl7:author" mode="EnteredByDetail"/>
			</xsl:when>
			<xsl:when test="hl7:assignedAuthor">
				<xsl:apply-templates select="." mode="EnteredByDetail"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates select="$defaultAuthorRootPath" mode="EnteredByDetail"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
			...
			<EnteredBy>
				<xsl:choose>
					<xsl:when test="hl7:assignedAuthor/hl7:code and ((not(hl7:assignedAuthor/hl7:code/@nullFlavor)) or (hl7:assignedAuthor/hl7:code/@nullFlavor and string-length(hl7:assignedAuthor/hl7:code/hl7:originalText/text())))">
						...
						<Code>
							<xsl:value-of select="$authorType"/>
						</Code>
						<Description>
							<xsl:value-of select="$authorType"/>
						</Description>
					...
					<xsl:otherwise>
						<xsl:variable name="contactNameXML">
							<xsl:apply-templates select="hl7:assignedAuthor/hl7:assignedPerson/hl7:name" mode="ContactName"/>
						</xsl:variable>
		...
		<OrderedBy>
			<xsl:choose>
				...
				<xsl:otherwise>
					...
					<Code>
						<xsl:value-of select="$orderedByCode"/>
					</Code>
					<Description>
						<xsl:value-of select="$contactNameFull"/>
					</Description>
					<xsl:if test="string-length($contactName/GivenName/text()) and string-length($contactName/FamilyName/text())">
						<Name>
							<xsl:if test="string-length($contactName/FamilyName/text())">
								<FamilyName>
									<xsl:value-of select="$contactName/FamilyName/text()"/>
								</FamilyName>
							</xsl:if>
							<xsl:if test="string-length($contactName/NamePrefix/text())">
								<NamePrefix>
									<xsl:value-of select="$contactName/NamePrefix/text()"/>
								</NamePrefix>
							</xsl:if>
							<xsl:if test="string-length($contactName/GivenName/text())">
								<GivenName>
									<xsl:value-of select="$contactName/GivenName/text()"/>
								</GivenName>
							</xsl:if>
							<xsl:if test="string-length($contactName/MiddleName/text())">
								<MiddleName>
									<xsl:value-of select="$contactName/MiddleName/text()"/>
								</MiddleName>
							</xsl:if>
							<xsl:if test="string-length($contactName/ProfessionalSuffix/text())">
								<ProfessionalSuffix>
									<xsl:value-of select="$contactName/ProfessionalSuffix/text()"/>
								</ProfessionalSuffix>
							</xsl:if>
						</Name>
					</xsl:if>
	...
	<xsl:template match="*" mode="CareProviderDetail">
		<!--
			...
			Path  : Description
			XPath : Description/text()
			Source: assignedPerson/name/text()
		...
		<xsl:if test="$entityPath = true()">
			...
			<xsl:if test="$personPath = true() and not($personPath/hl7:name/@nullFlavor)">
				...
				<xsl:variable name="clinicianAssigningAuthority">
					<xsl:apply-templates select="." mode="code-for-oid">
						<xsl:with-param name="OID" select="$entityPath/hl7:id/@root"/>
					</xsl:apply-templates>
				</xsl:variable>
				<xsl:variable name="clinicianID" select="$entityPath/hl7:id/@extension"/>
				<xsl:variable name="clinicianName">
					<xsl:apply-templates select="$personPath/hl7:name" mode="ContactNameString"/>
				</xsl:variable>
				...
				<xsl:if test="string-length($clinicianAssigningAuthority) and not($clinicianAssigningAuthority=$noCodeSystemOID) and not($clinicianAssigningAuthority=$noCodeSystemName)">
					<SDACodingStandard>
						<xsl:value-of select="$clinicianAssigningAuthority"/>
					</SDACodingStandard>
				</xsl:if>
				<Code>
					<xsl:choose>
						<xsl:when test="string-length($clinicianID)">
							<xsl:value-of select="$clinicianID"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="$clinicianName"/>
						</xsl:otherwise>
					</xsl:choose>
				</Code>
				<Description>
				...
				<xsl:if test="string-length($clinicianFunctionCode)">
					<CareProviderType>
						...
						<Code>
							<xsl:value-of select="$clinicianFunctionCode"/>
						</Code>
						<Description>
							<xsl:value-of select="$clinicianFunctionDesc"/>
						</Description>
					</CareProviderType>
				</xsl:if>
			</xsl:if>
	...
	<xsl:template match="*" mode="ContactName">
		...
		<xsl:if test="not(@nullFlavor)">
			...
			<xsl:variable name="contactNameGiven">
				<xsl:choose>
					<xsl:when test="string-length(hl7:given[1]/text())">
						<xsl:value-of select="hl7:given[1]/text()"/>
					</xsl:when>
			...
			<xsl:variable name="contactNameLast">
				<xsl:choose>
					<xsl:when test="string-length(hl7:family/text())">
						<xsl:value-of select="hl7:family/text()"/>
					</xsl:when>
					<xsl:when test="string-length(normalize-space(text())) and contains(text(), ',')">
						<xsl:value-of select="normalize-space(substring-before(text(), ','))"/>
					</xsl:when>
			...
			<xsl:element name="{$elementName}">
				<xsl:if test="string-length($contactNameLast)">
					<FamilyName>
						<xsl:value-of select="$contactNameLast"/>
					</FamilyName>
				</xsl:if>
				<xsl:if test="string-length($contactNamePrefix)">
					<NamePrefix>
						<xsl:value-of select="$contactNamePrefix"/>
					</NamePrefix>
				</xsl:if>
				<xsl:if test="string-length($contactNameGiven)">
					<GivenName>
						<xsl:value-of select="$contactNameGiven"/>
					</GivenName>
				</xsl:if>
				<xsl:if test="string-length($contactNameMiddle)">
					<MiddleName>
						<xsl:value-of select="$contactNameMiddle"/>
					</MiddleName>
				</xsl:if>
				<xsl:if test="string-length($contactNameSuffix)">
					<ProfessionalSuffix>
						<xsl:value-of select="$contactNameSuffix"/>
					</ProfessionalSuffix>
				</xsl:if>
			</xsl:element>
		</xsl:if>
	...
	<xsl:template match="*" mode="ContactNameString">
		<xsl:choose>
			...
			<xsl:when test="string-length(hl7:given[1]/text()) and string-length(hl7:given[2]/text()) and string-length(hl7:family/text())">
				<xsl:value-of select="concat(hl7:family/text(), ', ', hl7:given[1]/text(), ' ', hl7:given[2]/text())"/>
			</xsl:when>
			<xsl:when test="string-length(hl7:given[1]/text()) and string-length(hl7:family/text())">
				<xsl:value-of select="concat(hl7:family/text(), ', ', hl7:given[1]/text())"/>
			</xsl:when>
			<xsl:when test="string-length(hl7:family/text())">
				<xsl:value-of select="hl7:family/text()"/>
			</xsl:when>
			...
			<xsl:otherwise>
				<xsl:element name="{$elementName}">
					<xsl:if test="string-length(hl7:useablePeriod/hl7:low/@value)">
						<FromTime>
							<xsl:value-of select="isc:evaluate('xmltimestamp', hl7:useablePeriod/hl7:low/@value)"/>
						</FromTime>
					</xsl:if>
					<xsl:if test="string-length(hl7:useablePeriod/hl7:high/@value)">
						<ToTime>
							<xsl:value-of select="isc:evaluate('xmltimestamp', hl7:useablePeriod/hl7:high/@value)"/>
						</ToTime>
					</xsl:if>
		...
		<xsl:if test="hl7:telecom[not(@nullFlavor)]">
			<xsl:element name="{$elementName}">
				<xsl:for-each select="hl7:telecom[not(@nullFlavor)]">
					...
					<xsl:choose>
						<xsl:when test="contains($homeTelecomUse, $telecomUse)">
							<HomePhoneNumber>
								<xsl:value-of select="$telecomValue"/>
							</HomePhoneNumber>
						</xsl:when>
						<xsl:when test="contains($workTelecomUse, $telecomUse)">
							<WorkPhoneNumber>
								<xsl:value-of select="$telecomValue"/>
							</WorkPhoneNumber>
						</xsl:when>
						<xsl:when test="contains($mobileTelecomUse, $telecomUse)">
							<MobilePhoneNumber>
								<xsl:value-of select="$telecomValue"/>
							</MobilePhoneNumber>
						</xsl:when>
						<xsl:when test="contains($emailTelecomUse, $telecomUse)">
							<EmailAddress>
								<xsl:value-of select="$telecomValue"/>
							</EmailAddress>
						</xsl:when>
						<xsl:when test="not(string-length(@use)) and contains(@value,'mailto:')">
							<EmailAddress>
								<xsl:value-of select="$telecomValue"/>
							</EmailAddress>
						</xsl:when>
						<xsl:when test="not(string-length(@use))">
							<HomePhoneNumber>
								<xsl:value-of select="$telecomValue"/>
							</HomePhoneNumber>
						</xsl:when>
	...
	<xsl:template match="*" mode="OrganizationInformation">
		...
		<xsl:variable name="communityOID">
			<xsl:choose>
				<xsl:when test="$wholeOrganizationRootPath">
					<xsl:value-of select="$wholeOrganizationRootPath/hl7:id/@root"/>
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:variable name="communityCode">
			<xsl:apply-templates select="." mode="code-for-oid">
				<xsl:with-param name="OID" select="$communityOID"/>
			</xsl:apply-templates>
		</xsl:variable>
		<xsl:variable name="communityDescription">
			<xsl:choose>
				<xsl:when test="$wholeOrganizationRootPath">
					<xsl:value-of select="$wholeOrganizationRootPath/hl7:name/text()"/>
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:variable name="idRootExt">
			<xsl:apply-templates select="hl7:id" mode="representedOrganizationId"/>
		</xsl:variable>
		...
		<xsl:variable name="facilityCodeFromOID">
			<xsl:apply-templates select="." mode="code-for-oid">
				<xsl:with-param name="OID" select="$facilityOID"/>
			</xsl:apply-templates>
		</xsl:variable>
		<!--
	...
	<xsl:template match="*" mode="LocationInformation">
		...
		<xsl:variable name="locationCode">
			<xsl:apply-templates select="." mode="code-for-oid">
				<xsl:with-param name="OID" select="$locationOID"/>
			</xsl:apply-templates>
		</xsl:variable>
		...
		<xsl:if test="($sdaOverrideExternalId = 1)">
			<xsl:choose>
				<xsl:when test="hl7:id[contains(@assigningAuthorityName, 'ExternalId') and not(@nullFlavor)]">
					<ExternalId>
						<xsl:apply-templates select="hl7:id[contains(@assigningAuthorityName, 'ExternalId')]" mode="Id-External"/>
					</ExternalId>
				</xsl:when>
				<xsl:when test="hl7:id[(position() = 1) and not(@nullFlavor)]">
					<ExternalId>
						<xsl:apply-templates select="hl7:id[1]" mode="Id-External"/>
					</ExternalId>
				</xsl:when>
			</xsl:choose>
		</xsl:if>
	...
	<xsl:template match="*" mode="PlacerId">
		...
		<xsl:choose>
			<xsl:when test="hl7:id[contains(@assigningAuthorityName, '-PlacerId') and not(@nullFlavor)]">
				<PlacerId>
					<xsl:apply-templates select="hl7:id[contains(@assigningAuthorityName, '-PlacerId')]" mode="Id"/>
				</PlacerId>
			</xsl:when>
			...
			<xsl:otherwise>
				<PlacerId>
					<xsl:value-of select="isc:evaluate('createGUID')"/>
				</PlacerId>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template match="*" mode="FillerId">
		...
		<xsl:choose>
			<xsl:when test="hl7:id[contains(@assigningAuthorityName, '-FillerId') and not(@nullFlavor)]">
				<FillerId>
					<xsl:apply-templates select="hl7:id[contains(@assigningAuthorityName, '-FillerId')]" mode="Id"/>
				</FillerId>
			</xsl:when>

			<xsl:when test="hl7:id[(position() = 3) and not(@nullFlavor)]">
				<FillerId>
					<xsl:apply-templates select="hl7:id[3]" mode="Id"/>
				</FillerId>
			</xsl:when>
	...
	<xsl:template match="*" mode="PlacerApptId">
		<xsl:choose>
			<xsl:when test="hl7:id[contains(@assigningAuthorityName, '-PlacerApptId') and not(@nullFlavor)]">
				<PlacerApptId>
					<xsl:apply-templates select="hl7:id[contains(@assigningAuthorityName, '-PlacerApptId')]" mode="Id"/>
				</PlacerApptId>
			</xsl:when>
			<xsl:when test="hl7:id[(position() = 2) and not(@nullFlavor)]">
				<PlacerApptId>
	...
	<xsl:template match="*" mode="FillerApptId">
		...
		<xsl:choose>
			<xsl:when test="hl7:id[contains(@assigningAuthorityName, '-FillerApptId') and not(@nullFlavor)]">
				<FillerApptId>
					<xsl:apply-templates select="hl7:id[contains(@assigningAuthorityName, '-FillerApptId')]" mode="Id"/>
				</FillerApptId>
			</xsl:when>
			<xsl:when test="hl7:id[(position() = 3) and not(@nullFlavor)]">
				<FillerApptId>
	...
	<xsl:template match="*" mode="Id-External">
		...
		<xsl:choose>
			<xsl:when test="@extension">
				<xsl:value-of select="concat($rootCode, '|', @extension)"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="@root"/>
			</xsl:otherwise>
	...
	<xsl:template name="ActionCode">
		...
		<xsl:variable name="encounterNumberSubscript">
			<xsl:choose>
				<xsl:when test="string-length($encounterNumber)">
					<xsl:value-of select="$encounterNumber"/>
				</xsl:when>
			...
			<xsl:choose>
				<xsl:when test="($documentActionCode = 'APND')">
					<xsl:choose>
						...
						<xsl:when test="$informationType = 'Encounter'">
						...
						<xsl:when test="not(isc:evaluate('varGet','ActionCodes',$informationType,$encounterNumberSubscript)='1')">
							<xsl:element name="{$informationType}">
								<ActionCode>A</ActionCode>
								<xsl:if test="string-length($actionScope)">
									<ActionScope>
										<xsl:value-of select="$actionScope"/>
									</ActionScope>
								</xsl:if>
